...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
	 ...
	 * 4 5 6 7
	 *
	 * Los primos del elemento 4 son [6, 7] Los primos del elemento 7 son [4, 5]
	 * 
	 */

	// Considera que los padres son hermanos
	public List<Position<E>> cousins(Position<E> p) {
		List<Position<E>> cousins = new ArrayList<Position<E>>();

		Position<E> parent = parent(p);
		if (parent == null || parent(parent) == null)
			return cousins;

		for (Position<E> cl : children(sibling(parent)))
			cousins.add(0, cl);

		return cousins;
	}

	// Elementos al mismo nivel y distinto padre
	public List<Position<E>> cousins1(Position<E> p) {
		List<Position<E>> cousins = new ArrayList<Position<E>>();
		...
		int level = depth(p);
		for (Position<E> q : breadthfirst())
			if (depth(q) == level && !parent(q).equals(parent))
				cousins.add(0, q);
