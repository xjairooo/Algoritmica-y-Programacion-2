...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
	...
	public List<E> ListBinaryTree() {
		...
		List<Node<E>> p = new ArrayList<Node<E>>(maxNodos);
		for (int i = 0; i < maxNodos; i++)
			p.add(i, null);

		p.add(0, root);
		int recorrido = (int) Math.pow(2, altura) - 1;		
		for (int i = 0; i < recorrido; i++) {			
			if (p.get(i) != null) {				
				p.set(i * 2 + 1, p.get(i).left);
				p.set(i * 2 + 2, p.get(i).right);
			}
		}
		for (int i = 0; i < maxNodos; i++)
			if (p.get(i) != null)
				l.add(i, p.get(i).element);
			else
				l.add(i, null);
