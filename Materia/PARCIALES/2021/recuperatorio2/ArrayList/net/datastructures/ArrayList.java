...
public class ArrayList<E> implements List<E> {
	...
	public void addAllBad(int index, List<E> list) throws IndexOutOfBoundsException {
		checkIndex(index, size + 1);
		if (size + list.size() >= data.length)
			resize(data.length + list.size());
		for (int k = 0; k < list.size(); k++)
			add(index + k, list.get(k));
	}
	 ...
	 * @throws IndexOutOfBoundsException lanza la excepción si el índice esta fuera de rango (index < 0
	 *                                   || index > size)
	 */
	/**
	 * Ejemplo:
	 ...
	 */
	@Override
	public void addAll(int index, List<E> list) throws IndexOutOfBoundsException {
		checkIndex(index, size + 1);
		if (size + list.size() >= data.length)
			resize(data.length + list.size());
		for (int k = size - 1; k >= index; k--)
			data[k + list.size()] = data[k];
		for (int k = 0; k < list.size(); k++)
			data[index + k] = list.get(k);
		size += list.size();
	}
}
