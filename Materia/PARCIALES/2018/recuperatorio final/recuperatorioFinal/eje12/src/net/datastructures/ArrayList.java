...
public class ArrayList<E> implements List<E> {
	...
	/*
	 * Remueve de la lista todos los elementos que están contenidos en la
	 * colección especificada. Retorna true si la lista cambia como resultado de
	 * su llamada.
	 */
	...
	/*
	 * Retiene solo los elementos de la lista que están en la lista
	 * especificada. En otras palabras remueve de la lista todos los elementos
	 * que no están contenidos en la lista especificada. Retorna true si la
	 * lista cambia como resultado de su llamada.
	 */
	public boolean retainAll(List<E> l) {
		List<E> aux = new ArrayList<>();
		for (int k = 0; k < size; k++)
			if (l.indexOf(data[k]) == -1)
				aux.add(0, data[k]);
		if (aux.isEmpty())
			return false;
	...
	public boolean addAll(int i, List<E> l) throws IndexOutOfBoundsException {
		int s = size;
		for (E e : l)
			add(i++, e);
		return size != s;
	...
	public boolean addAll2(int i, List<E> l) throws IndexOutOfBoundsException {
		checkIndex(i, size + 1);
		int s = size;
		int c = l.size();
		if (size + c == data.length)
			resize(data.length + c);
		for (int k = size + c - 1; k >= i + c; k--)
			data[k] = data[k-c];
		for (E e : l)
			data[i++] = e;
		size += c;
