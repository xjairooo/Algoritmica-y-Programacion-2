...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
	...
	public Position<E> search(E p) {
		Iterable<Position<E>> l = inorder();
		for (Position<E> n : l)
			if (n.getElement().equals(p))
				return n;
		return null;
	}

	/*
	 * Retorna true si todos los nodos que no son hojas tienen dos hijos
	 */
	public boolean lleno() {
		Iterable<Position<E>> l = inorder();
		for (Position<E> n : l)
			if (isInternal(n) && numChildren(n) != 2)
				return false;
		return true;
	}
	...
	/*
	 * Dos árboles son semejante si tienen los mismos nodos
	 */
	public boolean semejante(LinkedBinaryTree<E> t) {
		if (size != t.size)
			return false;
		for (Position<E> n : t.inorder())
			if (search(n.getElement()) == null)
				return false;
		return true;
	}

	public int count(E p) {
		int c = 0;
		Iterable<Position<E>> l = inorder();
		for (Position<E> n : l)
			if (n.getElement().equals(p))
	...
	public boolean duplicado() {
		for (Position<E> n : inorder())
			if (count(n.getElement()) > 1)
				return true;
		return false;
	}

	/*		 
	 * Verifica si un árbol binario de expresiones se puede evaluar. 
	 */
