...
public class DoublyLinkedList<E> {
	...
	/* Retorna una nueva lista con los últimos (p) elementos de la lista */
	/* Si la acción no es válida lanza la excepción: IndexOutOfBoundsException */

	/*
	 * Por ejemplo dada la lista {A, B, C, D, E}
	 * 
	 * rightList(2) => {D, E} rightList(1) => {E} rightList(5) => {A, B, C, D,
	 * E} rightList(0) => {} rightList(6) => IndexOutOfBoundsException
	 * 
	 * Tener presente que este método retorna una lista nueva. No modifica la
	 * lista original.
	 */

	public DoublyLinkedList<E> rightList(int p) {
		if (p < 0 || p > size)
			throw new IndexOutOfBoundsException("Posición no válida: " + p);

		DoublyLinkedList<E> rl = new DoublyLinkedList<E>();
		int c = 0;
		Node<E> walk = trailer.getPrev();
		while (c < p) {
			rl.addFirst(walk.getElement());
			walk = walk.getPrev();
			c++;
		}
		return rl;
	...
	/*
	 * Optimizar el algorítmo para que relice el menor número de ciclos de
	 * repetición.
	 * 
	 * Si la posición donde se debe insertar el elemento es menor a la mitad del
	 * tañano de la lista (size /2) recorrer comenzando del inicio de la lista
	 * sino recorrer comenzando desde el final de la misma.
	 */

	public void addPos(E e, int n) throws IndexOutOfBoundsException {
		if (n < 1 || n > size + 1)
			throw new IndexOutOfBoundsException("Posición inválida: " + n);
		int p;
		Node<E> aux;
		if (n <= (size / 2)) {
			p = 1;
