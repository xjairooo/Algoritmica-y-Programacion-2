    ...
    public static void validateHTMLAttributes(String filePath) {
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = reader.readLine()) != null) {
                lineNumber++;
                Pattern pattern = Pattern.compile("<(\\w+)\\s+([^>]+)>"); // Crea un patron para analizar los elementos
                                                                          // que componen a la sintaxis de html
                Matcher matcher = pattern.matcher(line); // Utiliza un matcher para reconocer estos patrones en la linea
                                                         // obtenida
                ...
                while (matcher.find()) { // Intenta encontrar alguna coincidencia del patron
                    ...
                    if (!attributes.matches("(\\w+)=\"[^\"]*\"")) {
                        System.out.printf(
                                "Error en la línea %d, columna %d: Delimitador de atributo inválido en la etiqueta <%s>%n",
