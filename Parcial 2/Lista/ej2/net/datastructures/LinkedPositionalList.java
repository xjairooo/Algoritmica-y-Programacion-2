...
public class LinkedPositionalList<E> implements PositionalList<E> {
  ...
  public void reverse() {
    ...
    // left apunta al primer nodo real osea la cabeza. Despues de header
    Node<E> left = header.getNext();
    // right apunta al ultimo nodo real. Antes del trailer
    Node<E> right = trailer.getPrev();

    // Recorre la mitad de la lista, intercambiando los elementos los elementos
    // extremos hacia el centro
    for (int i = 0; i < size / 2; i++) {
      // guarda el elemento de la izq
      E temp = left.getElement();
      // invertimos los elementos de der a izq e izq a der.
      left.setElement(right.getElement());
      right.setElement(temp);

      // avanzamos hacia la derecha
      left = left.getNext();
      // retrocede right hacia la izquierda
      right = right.getPrev();
    }
  }
}
