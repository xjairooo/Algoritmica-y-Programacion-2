...
public class LinkedPositionalList<E> implements PositionalList<E> {
   ...
   * @param toIndex
   *                  elemento final exclusive (es uno menor a la posición dada)
   *
   ...
   *
   * Notar que elimina el elemento que está en la posición 2 (incluive) y no
  ...
  public void removeAll(int fromIndex, int toIndex) {
    ...
    if (fromIndex < 0 || toIndex > size)
      throw new IndexOutOfBoundsException("Indices fuera del rango");
    if (fromIndex == toIndex)
      return; // nada q remover

    // avanzar hasta el nodo en fromIndex
    Node<E> current = header.getNext();
    for (int i = 0; i < fromIndex; i++) {
      current = current.getNext();
    }

    // guardar el nodo toIndex
    Node<E> end = current;
    for (int i = fromIndex; i < toIndex; i++) {
      end = end.getNext();
    }

    // desconectar los nodos a eleminar
    Node<E> before = current.getPrev();
    before.setNext(end);
    end.setPrev(before);

    // actualiza el size y limpiar los nodos eleminados
