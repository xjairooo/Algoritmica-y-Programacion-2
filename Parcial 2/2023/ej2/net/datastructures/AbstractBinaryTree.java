...
public abstract class AbstractBinaryTree<E> extends AbstractTree<E>
  ...
  public boolean complete() {
    ...
    int i = 0;
    while (i < list.size()) {
      Position<E> p = list.get(i++);
      if (p == null) {
        // si hay un null, todos los siguientes deben ser null
        while (i < list.size()) {
          if (list.get(i++) != null)
            return false;
        }
    ...
    // el arbol es completo si la lista tiene exactamente n nodos no nulos en las
    // primeras posiciones
    for (int j = 0, count = 0; j < list.size() && count < n; j++) {
      if (list.get(j) != null)
        count++;
      else
        return false;
