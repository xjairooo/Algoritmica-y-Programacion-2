...
public class GraphAlgorithms {
   ...
   * @param forest is a map from nonroot vertex to its discovery edge in DFS
   *               forest
   *
   *               As an outcome, this method adds newly discovered vertices
   *               (including u) to the known set,
   ...
   * @param forest must be a map that resulting from a previous call to DFS
   *               started at u.
   */
  ...
  /**
   * Performs breadth-first search of the undiscovered portion of Graph g starting
   * at Vertex s.
   *
   * @param g      Graph instance
   ...
   *
   * @return map such that each nonroot vertex v is mapped to its discovery edge
   *         (vertices that are roots of a BFS trees in the forest are not
   *         included in the map).
   */
  ...
  /**
   * Computes a minimum spanning tree of connected, weighted graph g using
   * Kruskal's algorithm.
   *
   * Result is returned as a list of edges that comprise the MST (in arbitrary
   * order).
   */
